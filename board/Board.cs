            ...
            foreach (Tile tile in Tiles)
            {
                if (tile.OnClick(position))
                {
                    ...
                    if (tile.Piece != null && tile.Piece.SideOfPiece.Equals(_sideOfPlayer))
                    {
                        ...
                        PositionSelected.Piece.CalculateRegularMoves(tile).ForEach(position =>
                        {
        ...
        public void AwaitReplyFromServer(Tile tile)
        {
            ...
            if (GotReply)
            {
                ...
                if(PositionSelected.Piece != null && PositionSelected.Tile != null)
                {
                    ...
                    //Console.WriteLine($"NEW POSITION: {Tiles[tile.GetPositionInTilesArray()].GetPositionInTilesArray()}");
